document.addEventListener('DOMContentLoaded', () => {
    ...
    function triggerHaptic() {
        ...
        if (window.navigator && window.navigator.vibrate) {
            // Используем массив [50], это надежнее. 
            // 50мс - это ощутимая вибрация (не слишком короткая)
    ...
    hapticButtons.forEach(btn => {
        // Используем 'pointerdown' вместо 'click'.
        // Это срабатывает МГНОВЕННО при касании экрана, еще до того как палец отпущен.
        // Ощущается гораздо приятнее и работает стабильнее.
